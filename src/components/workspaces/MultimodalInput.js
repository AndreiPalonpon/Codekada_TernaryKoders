...
import { ChevronDown, ChevronUp, Upload, Link as LinkIcon, Sparkles, Loader2, AlertTriangle, X, FileImage, Link2, FileText } from "lucide-react";
...
export default function MultimodalInput({ isExpanded = true, onToggle, embedMode = false }) {
  ...
  useEffect(() => {
    fetch("/api/ai/config")
      .then((res) => res.json())
      .then((data) => {
        if (data.success && data.modelName) {
          setAiModelName(data.modelName);
        }
      })
      .catch(() => {
        setAiModelName("AI Model");
      });
  }, []);
  ...
  // Browser-based PDF text extractor using pdf.js loaded dynamically from Cloudflare CDN
      ...
      try {
        if (isImage) {
          ...
          try {
            ...
            console.warn("PDF extraction failed, falling back to base64 encoding", pdfErr);
            const fallbackReader = new FileReader();
            fallbackReader.onload = (fbEvent) => {
              addAttachment({
                type: "document_base64",
                content: fbEvent.target.result,
                name: file.name,
      ...
      {attachments.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-2">
          {attachments.map((att, idx) => {
            let IconComp = FileImage;
            if (att.type === "link") IconComp = Link2;
            else if (att.type === "document" || att.type === "document_base64") IconComp = FileText;
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-2 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors group relative disabled:opacity-50"
            title={isParsing ? "Extracting Text..." : "Upload File"}
          >
            {isParsing ? <Loader2 size={16} className="animate-spin text-emerald-600" /> : <Upload size={16} />}
          </button>
          <button
            type="button"
        ...
        <button
          ...
          className={`px-5 py-2.5 text-sm font-semibold rounded-lg transition-all shadow-md flex items-center gap-2 group ${isLoading || (!textInput.trim() && attachments.length === 0)
